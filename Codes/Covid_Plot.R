PO_Covid<-function(dataset)
{
  dataset%>% 
    ...
    annotate("text", x = dataset$Date[dim(dataset)[1]-50], y = min(round(dataset[,1],2)), label = "Training ",size=5,family="Times New Roman")+
    annotate("text", x = dataset$Date[dim(dataset)[1]-2], y = min(round(dataset[,1],2)), label = "Test",size=5,family="Times New Roman")+
